...
public class ShellCommand {
    ...
    public class SH {
        ...
        public Process run(final String s) {
            ...
            try {
                process = Runtime.getRuntime().exec(SHELL);//系统已经开放了root权限，而不是通过执行这句来获得root权限
                DataOutputStream toProcess = new DataOutputStream(process.getOutputStream());
                toProcess.writeBytes("exec " + s + "\n");
                toProcess.flush();
            } catch(Exception e) {
                //若已经root，但是用户选择拒绝授权，抛出异常Java.io.IOException:write failed:EPIPE(Broken pipe)
                //如果没有root，Error running exec(). Command: [su] Working Directory: null Environment: null
